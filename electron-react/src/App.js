...
const App = () => {
  ...
  useEffect(() => {
    const handleMessage = (receivedMessage) => {
      ...
      // Asegúrate de que receivedMessage tiene las propiedades 'emisor' y 'mensaje'
      if (receivedMessage && receivedMessage.message !== undefined && receivedMessage.userName !== undefined) {
        addMessage({
          ...
          direction: 'received', // Asumimos que todos los mensajes recibidos son de otros usuarios
        });
      } else {
  ...
  useEffect(() => {
    // Aquí selectedUserName siempre estará actualizado
    console.log(selectedUserName);
    // Realiza las acciones que necesitas con selectedUserName actualizado
  }, [selectedUserName]); // Este efecto se ejecutará cada vez que selectedUserName cambie
  ...
  const handleBackgroundChange = (event) => {
    setBackground(`url("url("resources/Fondo_chat.webp")")`);
  ...
  const sendMessage = () => {
    ...
    const selectedUser = usersData[selectedUserName];
    if (!selectedUser) {
      console.error('No user selected');
      return;
    }

    // Crear el objeto de mensaje para los mensajes enviados
    const newMessage = {
      sender: "alexalameda",
      text: messageToSend,
      direction: "sent",
    ...
    // Preparar los datos para enviar al proceso principal
    const data = {
      userName: selectedUserName,
    ...
    // Envía el mensaje, el nombre de usuario y la IP al proceso principal
  ...
  function ButtonPage({ ipAddress }) {
    const getValuesForm = () => {
      ...
      window.appCommunication.sendMessageIPC(data);
    }
      ...
      >
        ...
        <div className="chat-info">
          ...
          <p className="chat-status">
            {userData.status ? 'Online' : `Última vez: ${userData.lastConnectionTime.date} ${userData.lastConnectionTime.time}`}
          </p>
        </div>
      </li>
  );
